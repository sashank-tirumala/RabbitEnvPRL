...
class Serial2RKin():
    ...
    def inverseKinematics(self, ee_pos, branch=1):
        ...
        current_reach = np.round((x**2 + y**2),4)
        max_reach = np.round((l1+l2)**2,4)
        min_reach = np.round((l1-l2)**2,4)

        if ( current_reach>  max_reach or current_reach < min_reach):
            #print("Point is outside the workspace")
            # print('x:',x,'y:',y,"\nx**2 + y**2",x**2 + y**2,'\n(l1+l2)**2)',(l1+l2)**2,'(l1-l2)**2',(l1-l2)**2)
            valid=False
            print("invalid")
            q[0] = -np.pi/2
            q[1] = 0.0
            return valid, q[0],q[1]


        a = np.round(2*l2*x , 4)
        b =np.round( 2*l2*y , 4)
        c = np.round(l1**2 - l2**2 - x**2 - y**2 , 4)
        q1_temp = 0
        if branch == 1:
